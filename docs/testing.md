...
- **One Zorro II autoconfig slot**, nibble packed as the bus is, with
  `er_Type` read straight and every other register presented inverted. The
  card stops answering once it is relocated or shut up. With nothing attached
  the space reads `$FF`, which is the "no card" answer.
- **Stubs** that exist only to stop routines hanging: CIA-A and CIA-B.
...
guard the fix. The suite is 148 tests, no xfails.
