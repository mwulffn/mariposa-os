...
def verify(img):
    ...
    if (root_start, root_secs, data_start) != (ROOT_DIR_START, ROOT_DIR_SECS, DATA_START):
    ...
    for s in range(root_secs):
        ...
        for o in range(0, bps, 32):
            ...
            if e[0:11] == b'SYSTEM  BIN':
                found = (struct.unpack_from('<H', e, 26)[0],
...
def main(argv):
    ...
    with open(os.path.join(out, 'disk_layout.h'), 'w') as f:
        f.write(HEADER % {
            'total_sectors': TOTAL_SECTORS, 'heads': HEADS, 'sectors': SECTORS,
            'lowcyl': LOWCYL, 'highcyl': HIGHCYL,
            'part_start': PART_START, 'part_size': PART_SIZE,
            'bps': BYTES_PER_SEC, 'spc': SEC_PER_CLUS, 'rsvd': RSVD,
            'nfats': NUM_FATS, 'rootent': ROOT_ENT, 'fatsz': FAT_SIZE,
            'root_start': ROOT_DIR_START, 'root_secs': ROOT_DIR_SECS,
            'data_start': DATA_START,
            'sysbin_size': SYSBIN_SIZE, 'nclusters': len(SYSBIN_CHAIN),
