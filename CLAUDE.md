...
```bash
make test                      # headless, 360 tests, ~0.3s, no emulator needed
...
`ps`, `dev`, `irq`, `mount`, `ls`, `cat`, `bcache`, `keys` (watch keyboard
...
- Storage is in, read side (`docs/fs_design.md`): block devices and RDB
  partitions, a block cache, a VFS with Amiga-shaped names, FAT16 read-only.
  The kernel mounts the partition it booted from as `boot:`. **Next: ext2,
  read-write in the kernel and read-only in the ROM**, which needs an ext2
  test image; e2fsprogs is assumed to be installed, with a clear error if
  it is not (it must be added to `docker/Dockerfile` and the install scripts).
  The plan is FAT16 (r) + ext2 (r) in the ROM, FAT16 (r) + ext2 (r/w) in the
  kernel, PFS3 later.
